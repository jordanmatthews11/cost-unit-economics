...
async function getExpensesByUserId(userId, statusFilter = null, categoryFilter = null, yearFilter = null, monthFilter = null, vendorFilter = null, periodFilter = null) {
  ...
  if (periodFilter != null && Array.isArray(periodFilter) && periodFilter.length > 0) {
    const allowed = new Set(periodFilter.map((p) => String(p).trim()).filter(Boolean));
    rows = rows.filter((r) => {
    ...
    if (monthFilter != null && (Array.isArray(monthFilter) ? monthFilter.length > 0 : String(monthFilter).trim() !== '')) {
      const allowed = Array.isArray(monthFilter)
        ? monthFilter.map((m) => (String(m).trim().length === 1 ? '0' + String(m).trim() : String(m).trim()))
        : [(String(monthFilter).trim().length === 1 ? '0' + String(monthFilter).trim() : String(monthFilter).trim())];
      rows = rows.filter((r) => allowed.includes((r.date || '').slice(5, 7)));
    }
  }
...
/**
 * One document per user: team_costs/{userId}
 * Stores the full client unit economics blob under unit_economics.
 */
async function getTeamCostsByUserId(userId) {
  const firestore = getFirestore();
  const snap = await firestore.collection(TEAM_COSTS).doc(userId).get();
  if (!snap.exists) return null;
  const data = snap.data();
  const ue = data && data.unit_economics;
  if (!ue || typeof ue !== 'object') return null;
  return ue;
}

async function upsertTeamCosts(userId, unitEconomics) {
  const firestore = getFirestore();
  const ref = firestore.collection(TEAM_COSTS).doc(userId);
  const now = admin.firestore.FieldValue.serverTimestamp();
  await ref.set(
    {
      user_id: userId,
